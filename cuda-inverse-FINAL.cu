...
//"It looks like there is a race situation -- I got the same result in my solution. So I won't deduct points if your solution does not work for
//these special cases. The CUDA scheduler will run 32 threads at once." --- Dr. Cheung.

//My code works perfectly for all matrices but matrices with these dimensions.
...
__global__ void inverse( float *A, float *C, int N)
{
        ...
        //printf("block id (%d, %d), thread id (%d, %d)\n",blockIdx.x, blockIdx.y, threadIdx.x,threadIdx.y);
         ...
         for (int T = 0; T < N; T++)
         {
            //__syncthreads(); <-- Works for matrix sizes (1x1)-(90x90) approximately, but not for larger matrix sizes, which I think are more important.
               if (T == R)
               {
                  // Do nothing to row "R"
               }
         ...
         //__syncthreads(); <-- Works for large matrix sizes, but not smaller matrix sizes like 33*33. Works for (1x1)-(32x32) and (500x500) onwards

         //Without the synthreads(), there are less race conditions in my code (that are guaranteed to happen since we haven't 
         //learned yet how to use locks to our advantage with CUDA).
         // This is why I chose not to use the syncthread() method.
         //My code calculates the inverses of the matrices correctly for a greater range of dimensions without the syncthread() method.
      }
}
