...
__global__ void inverse( float *A, float *C, int N)
{
        ...
        atomicExch(&A[R*N+col], A[R*N+col]/factor); //divide A(:,col) by its diagonal
        atomicExch(&C[R*N+col], C[R*N+col]/factor); 
        ...
        for (int T = 0; T < N; T++)
        {
            ...
            else
            {
                ...
                ------------------------------------- */
                atomicAdd(&A[T*N+col],-f*A[R*N+col]); //add -A[T,R]*A[R,col] to A[T,col]
                atomicAdd(&C[T*N+col],-f*C[R*N+col]);
            }
        }
    }
